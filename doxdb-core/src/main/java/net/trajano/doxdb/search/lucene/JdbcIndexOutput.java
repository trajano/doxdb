        ...
        try (PreparedStatement lock = connection.prepareStatement(String.format("select name from %1$s where name = ? for update", searchTableName))) {
            ...
            try (ResultSet rs = lock.executeQuery()) {
                ...
                if (!rs.next()) {
                    ...
                    statement = connection.prepareStatement(String.format("insert into %1$s (content,contentlength, name) values (?, ?, ?)", searchTableName));
                    statement.setString(3, name);
                } else {
                    statement = connection.prepareStatement(String.format("update %1$s set content = ?, contentlength = ? where name=?", searchTableName));
                    statement.setString(3, name);
